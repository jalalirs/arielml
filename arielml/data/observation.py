...
import numpy as np
import time
from . import loaders, calibration, photometry, detrending
...
class DataObservation:
    ...
    def __init__(self, planet_id: str, instrument: str, obs_id: int, split: str = "train"):
        ...
        self.detrended_light_curves = None # To store final detrended data
        self.mask = None
        self.star_info = None
    ...
    def load(self, backend: str = 'cpu'):
        """
        Loads all data from disk and moves it to the specified backend device (CPU/GPU).
        """
        if self.backend_name == 'gpu':
            print("Clearing GPU memory pool...")
            self.xp.get_default_memory_pool().free_all_blocks()
        
        self.raw_signal = None; self.processed_signal = None; self.light_curves = None; self.calib_data = {}; self.detrended_light_curves = None
    ...
    def get_time_array(self) -> np.ndarray:
        """Generates the time array for the observation in days."""
        if self.processed_signal is None: return np.array([])
        num_frames = self.processed_signal.shape[0]
        timestep_days = 0.1 / (24 * 3600) 
        return np.arange(num_frames) * timestep_days

    def get_transit_mask(self) -> "xp.ndarray":
        """Calculates the transit mask using stellar parameters on the current backend."""
        if self.star_info is None:
            raise ValueError("Star info not loaded, cannot calculate transit mask.")
    ...
    def run_calibration_pipeline(self, steps_to_run: dict = None):
        ...
        if steps_to_run.get('mask'):
            ...
            _, self.mask = calibration.mask_hot_dead_pixels(signal_before_mask, self.calib_data["dead"], self.calib_data["dark"], xp=self.xp)
        ...
        if steps_to_run.get('flat'):
            if 'Correlated Double Sampling' in " ".join(self.calibration_log): self.processed_signal = self._apply_timed_step(calibration.apply_flat_field, self.processed_signal, self.calib_data["flat"], step_name="Flat Field Correction", xp=self.xp)
            else: self.calibration_log.append("Skipping Flat Field: CDS must be applied first.")
    ...
    def run_detrending(self, detrender: detrending.BaseDetrender):
        """
        Runs a specified detrending model on the extracted light curves.
        """
        ...
        self.detrended_light_curves = self._apply_timed_step(
            detrender.detrend,
            time_array_xp,
            self.light_curves,
            transit_mask,
            step_name=f"Detrending ({detrender.__class__.__name__})",
            xp=self.xp
        )
        return self.calibration_log
    ...
    def get_detrended_light_curves(self, return_type='numpy'):
        """Returns the final, detrended 1D light curves."""
        if self.detrended_light_curves is None:
            return None
        if return_type == 'numpy':
            if hasattr(self.detrended_light_curves, 'get'):
                return self.detrended_light_curves.get()
