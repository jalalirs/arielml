...
class DataObservation:
    ...
    def run_phase_folding(self, n_bins: int = 100):
        """
        Phase-folds the final detrended light curve to reveal the transit signal.
        """
    ...
    def get_phase_folded_lc(self):
        """Returns the phase-folded light curve data."""
    ...
    def get_transit_mask(self) -> "xp.ndarray":
        ...
        time_array = self.get_time_array(); time_array_xp = self.xp.asarray(time_array)
        return utils.calculate_transit_mask(time_array_xp, period=self.star_info['P'], semi_major_axis=self.star_info['sma'], stellar_radius=self.star_info['Rs'], inclination=self.star_info['i'], xp=self.xp)
